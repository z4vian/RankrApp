 ...
 * Followers list for the profile owner identified by [username].
 */
...
/**
 * Locally-narrowed FollowUser — username is required for navigation and display.
 * Rows without a username (legacy pre-onboarding profiles) are filtered out
 * at load time so the renderer can treat username as a string.
 */
...
export default function FollowersScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setNotFound(false);
    const rawUsers = await getFollowers(profile.id, 50);
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          subtitle="Be the first to spread the word!"
        />
      ) : (
        <ScrollView
          style={{ flex: 1 }}
        ...
        >
          {rows.map((row, i) => (
            <UserRow
              ...
              // username is guaranteed non-null by the LinkableUser filter
