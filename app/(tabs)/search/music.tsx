  ...
  const renderTrack = ({ item }: { item: Track }) => (
    <TouchableOpacity
      ...
      onPress={() => openSheet({
        ...
        external_id: item.trackId.toString(),
        category: 'music',
      })}
      activeOpacity={0.8}
    >
            ...
            <>
              ...
              {sheetStep === 'pick_list' && (
                <>
                  ...
                  {userLists.length === 0 ? (
                    <Text style={styles.noLists}>No music lists found. Create one in the Lists tab!</Text>
                  ) : (
                    userLists.map(list => (
                      <TouchableOpacity
                        key={list.id}
...
const styles = StyleSheet.create({
  ...
  noImage: { width: 64, height: 64, backgroundColor: '#111', justifyContent: 'center', alignItems: 'center' },
  info: { flex: 1, padding: 12 },
  title: { color: '#fff', fontSize: 14, fontWeight: '600', marginBottom: 2 },
  artist: { color: '#666', fontSize: 12, marginBottom: 2 },
  album: { color: '#555', fontSize: 11 },
  ...
  sheetImagePlaceholder: { width: 64, height: 64, backgroundColor: colors.card, borderRadius: 10, justifyContent: 'center', alignItems: 'center' },
