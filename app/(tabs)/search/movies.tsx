...
export default function MoviesSearch() {
  ...
  const handleSave = async () => {
    ...
    if (ranked.length === 0) {
      // First 10 items — assign evenly spaced ranks in sentiment range
    ...
    // 10+ ranked items — start binary comparison in sentiment range
    const range = sentimentRange(sentiment);
    ...
    if (pool.length === 0) {
      // No items in this sentiment range yet — assign midpoint of range
    ...
    // Start binary comparison within sentiment pool
    const lo = 0;
  ...
  const renderMovie = ({ item }: { item: Movie }) => (
    ...
    >
      ...
      <View style={styles.info}>
        ...
        <Text style={styles.year}>{item.release_date ? item.release_date.slice(0, 4) : 'Unknown year'}</Text>
            ...
            <>
              ...
              {sheetStep === 'pick_list' && (
                <>
                  ...
                  {userLists.length === 0 ? (
                    <Text style={styles.noLists}>No movie lists found. Create one in the Lists tab!</Text>
                  ) : (
                    userLists.map(list => (
                      <TouchableOpacity
                        key={list.id}
