/**
 * app/(tabs)/search/tv.tsx
 *
 * Phase 5 — TV show search. Same flow as movies/books/games/music:
 *   search → sentiment → pick list → notes/photos → comparison sheet.
 *
 * Data source: TMDB /search/tv (uses existing TMDB_API_KEY).
 */
...
/** TMDB /search/tv result shape (subset). */
type Show = {
  id: number;
  name: string;
...
export default function TVSearch() {
  ...
  const renderShow = ({ item }: { item: Show }) => {
    ...
    const posterFull = item.poster_path
    ...
    return (
      ...
      >
        ...
        <View style={styles.info}>
          ...
          <Text style={styles.year}>{year || 'Unknown year'}</Text>
    ...
    <View style={{ flex: 1 }}>
      <View style={styles.container}>
        ...
        {!loading && query.trim() && results.length === 0 && (
          <Text style={styles.noResults}>No TV shows found.</Text>
        )}
        <FlatList
          data={results}
            ...
            <>
              ...
              {sheetStep === 'pick_list' && (
                <>
                  ...
                  {userLists.length === 0 ? (
                    <Text style={styles.noLists}>No TV lists found. Create one in the Lists tab!</Text>
                  ) : (
                    userLists.map(list => (
                      <TouchableOpacity
                        key={list.id}
