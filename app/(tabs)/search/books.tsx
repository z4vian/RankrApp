...
/** Upgrade http→https for Google Books thumbnails (they serve mixed-protocol). */
const upgradeThumb = (url: string | undefined): string => {
  if (!url) return '';
...
export default function BooksSearch() {
  ...
  useEffect(() => {
    ...
    debounceTimer.current = setTimeout(() => searchBooks(query), 500);
  ...
  const searchBooks = async (searchQuery: string) => {
    ...
    setError('');
    try {
      // Phase 5 bug fix: drop the `intitle:` prefix. With `intitle:` the API
      // only matches title text, so a query like "stephen king" returns zero
  ...
  const renderBook = ({ item }: { item: Book }) => {
    ...
    const thumb = upgradeThumb(info.imageLinks?.thumbnail);
    return (
      <TouchableOpacity
        style={styles.card}
        onPress={() => openSheet({
      ...
      >
        ...
        <View style={styles.info}>
          ...
          <Text style={styles.year}>{authors || 'Unknown author'}</Text>
          {info.description ? (
            <Text style={styles.overview} numberOfLines={2}>{info.description}</Text>
          ) : null}
        </View>
      </TouchableOpacity>
    );
            ...
            <>
              ...
              {sheetStep === 'pick_list' && (
                <>
                  ...
                  {userLists.length === 0 ? (
                    <Text style={styles.noLists}>No book lists found. Create one in the Lists tab!</Text>
                  ) : (
                    userLists.map(list => (
                      <TouchableOpacity
                        key={list.id}
...
const styles = StyleSheet.create({
  ...
  year: { color: '#666', fontSize: 12, marginBottom: 4 },
  overview: { color: '#555', fontSize: 12 },
