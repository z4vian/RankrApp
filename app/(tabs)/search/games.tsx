  ...
  const searchGames = async (searchQuery: string) => {
    ...
    try {
      const response = await fetch(
        `https://api.rawg.io/api/games?key=${RAWG_API_KEY}&search=${encodeURIComponent(searchQuery)}&page_size=20`
      );
...
const styles = StyleSheet.create({
  ...
  sheetImagePlaceholder: { width: 80, height: 56, backgroundColor: colors.card, borderRadius: 10, justifyContent: 'center', alignItems: 'center' },
